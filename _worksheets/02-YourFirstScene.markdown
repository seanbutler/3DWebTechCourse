...
The array is a flexible versatile container, we can use it like a list or a table and also like a stack. There are methods for each way of accessing it. For now, to add something to the array we say 'push' which adds a variable to the top and also grows the array by one. Bit like a stack of plates.
